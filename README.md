...
## Variáveis automáticas nas mensagens do WhatsApp
Na mensagem-padrão de cada segmento, o painel reconhece automaticamente:

- `[NOME DA EMPRESA]` → nome salvo no contato.
- `[NOME DO VENDEDOR]` → vendedor responsável pelo estado.
- `[CIDADE]` → cidade do contato.

Os formatos antigos `{nome}`, `{vendedor}` e `{cidade}` continuam funcionando.

Exemplo:

```text
Olá! Meu nome é [NOME DO VENDEDOR] e trabalho com criação de sites para negócios locais.

Encontrei a [NOME DA EMPRESA] pesquisando por advogado no Google e vi que vocês ainda não têm um site próprio.
```

Se o vendedor for Raquel e o contato estiver salvo como `Almeida & Santos Advocacia`,
o link do WhatsApp será aberto com esses nomes já substituídos.
